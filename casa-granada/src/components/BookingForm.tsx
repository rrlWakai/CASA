...
export default function BookingForm() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!available) {
        toast.error('Sorry, those dates are already booked. Please choose different dates.');
        setLoading(false);
        return;
      }
  ...
  if (success) {
    return (
      ...
      >
        ...
        <h3 className="font-display text-3xl font-semibold text-gray-900 mb-3">Request Received!</h3>
        <p className="text-gray-500 mb-2">Thank you, <strong>{form.name}</strong>! Your booking request has been submitted.</p>
        <p className="text-gray-400 text-sm mb-8">We'll confirm your reservation via email or phone within 24 hours.</p>
        {form.checkIn && form.checkOut && (
          <div className="inline-flex gap-6 bg-emerald-50 rounded-2xl px-8 py-4 border border-emerald-100 mb-8">
            <div className="text-center">
              ...
              <div className="font-semibold text-gray-900">{format(form.checkIn, 'MMM d, yyyy')}</div>
            </div>
            <div className="w-px bg-emerald-200" />
            <div className="text-center">
              <div className="text-xs text-gray-400 uppercase tracking-wide mb-1">Check-out</div>
              <div className="font-semibold text-gray-900">{format(form.checkOut, 'MMM d, yyyy')}</div>
            </div>
            <div className="w-px bg-emerald-200" />
            <div className="text-center">
              <div className="text-xs text-gray-400 uppercase tracking-wide mb-1">Guests</div>
              <div className="font-semibold text-gray-900">{form.guests}</div>
            </div>
          </div>
        )}
        <button
          onClick={() => { setSuccess(false); setStep(1); setForm({ name: '', email: '', phone: '', checkIn: null, checkOut: null, guests: 2, specialRequests: '' }); }}
      ...
      <div className="flex items-center justify-center gap-3 mb-8">
        {[1, 2].map(s => (
          <div key={s} className="flex items-center gap-2">
            ...
            <span className={`text-sm font-medium ${step >= s ? 'text-gray-700' : 'text-gray-400'}`}>
              {s === 1 ? 'Select Dates' : 'Your Details'}
            </span>
            {s < 2 && <div className={`w-8 h-px mx-2 ${step > s ? 'bg-emerald-300' : 'bg-gray-200'}`} />}
          </div>
        ))}
      </div>
        ...
        >
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <DatePicker
                ...
                onChange={(date) => setForm(f => ({ ...f, checkIn: date, checkOut: f.checkOut && date && f.checkOut <= date ? null : f.checkOut }))}
                minDate={new Date()}
            ...
            <div>
              ...
              <DatePicker
                ...
                minDate={form.checkIn ? addDays(form.checkIn, 1) : addDays(new Date(), 1)}
                filterDate={(date) => !isDateBooked(date)}
          ...
          {nights > 0 && (
            ...
            >
              ...
              <span className="text-emerald-600 text-sm">
                {form.checkIn && format(form.checkIn, 'MMM d')} → {form.checkOut && format(form.checkOut, 'MMM d, yyyy')}
              </span>
            </motion.div>
          )}
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="font-display text-2xl font-semibold text-gray-900 w-8 text-center">{form.guests}</span>
              <button
      ...
      {step === 2 && (
        ...
        >
          ...
          <p className="text-center text-gray-400 text-xs">
            Your booking is a request — we'll confirm availability and send rates within 24 hours.
          </p>
        </motion.div>
      )}
    </div>
